...
public final class Maze implements Serializable {
    ...
    private final IMazeElements[][] board;

    public Maze(int height, int width) {
        board = new IMazeElements[height][width];
    }

    public boolean set(int y, int x, IMazeElements element) {
    ...
    public IMazeElements get(int y, int x) {
        if (y < 0 || y >= board.length || x < 0 || x >= board[0].length)
            return null;
